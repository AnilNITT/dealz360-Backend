...
// Generate OR with center Image
// app.get("/generateQR", async(req, res) => {
//     try {
//         const width = 400;
//         const cwidth = 80;
//         //  const {data} = req.body
//         const data = "hello jitu bhai";
//         if (data === null || data === "") {
//             response = {
//                 status: 201,
...
//         // QRCode.toDataURL(data,qrOption,async(error, url)=>{
...
//             // const center_image = "https://github.com/AnilNITT/AnilNITT/blob/master/image/IMG_20220914_140541_163-modified.png?raw=true";
...
async function GetMutualFollowingUser(req, res) {
  try {
    const datas = jwt_decode(req.headers.token);
    const user_id = datas.user_id;

    const data = await User.findById(user_id).populate({ path: "following", select: ["name","email", "username",'images'] })
    const user = await User.findById(req.params.id).populate({ path: "following", select: ["name","email", "username",'images'] })
    ...
    if(data._id.toString() === user._id.toString()){
      var response = {
        status: 201,
        message: "U can't Find Mutual Followed for urself",
      };
      return res.status(201).send(response);
    }
